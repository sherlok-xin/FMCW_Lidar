...
SPLIT_FILES = {
    ...
    "test": [".bin", ".label", ".invalid", ".occluded"]
}
...
class VoxelGeneratorWrapper():
    def __init__(self, vsize_xyz, coors_range_xyz, num_point_features, max_num_points_per_voxel, max_num_voxels):
        ...
        if self.spconv_ver == 1:
            ...
            self.voxel_size, _, g_, self.point_cloud_range = SpconvOps.calc_point2voxel_meta_data(
...
class get_dataset(Dataset):
    ...
    def __getitem__(self, t):
        ...
        # voxels, coords, num_points_per_voxel = self.voxel_generator(torch.concatenate([torch.from_numpy(xyz), torch.from_numpy(np.arange(len(xyz))).reshape(-1,1)],-1))
        # print(voxels.shape)
        # print(coords[:,::-1])
        # print(num_points_per_voxel.shape)
        # print(self.voxel_generator.vsize)
        # print(self.voxel_generator.coors_range)
        voxels, coords, num_points_per_voxel = self.voxel_generator.generate(np.concatenate([xyz, np.arange(len(xyz)).reshape(-1,1)],-1))
