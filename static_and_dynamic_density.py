...
class OnlineDynamicFilter:
    ...
    def _apply_neighborhood_filter(self, dynamic_mask, lin_idx, valid_mask):
        ...
        # 创建动态体素网格
        dynamic_grid = np.zeros((self.nx, self.ny, self.nz), dtype=bool)
        ...
        for i in dynamic_indices:
            ...
            # 修订后的邻域统计
            occupied_total = 0    # 被占据的体素总数（静态+动态）
            static_total = 0      # 静态体素数量
            
            # for dx in [-1, 0, 1]:
            #     for dy in [-1, 0, 1]:
            #         nx, ny, nz = ix + dx, iy + dy, iz
            #         if 0 <= nx < self.nx and 0 <= ny < self.ny and 0 <= nz < self.nz:
            #             # 检查该体素是否被占据（在背景模型中有记录）
            #             neighbor_idx = nx * self.ny * self.nz + ny * self.nz + nz
            #             is_occupied = (neighbor_idx in self.voxel_count)
                        
            #             if is_occupied:
            #                 occupied_total += 1
            #                 # 如果不是动态体素，则视为静态体素
            #                 if not dynamic_grid[nx, ny, nz]:
            #                     static_total += 1
            
            # # 核心修改：当静态体素占比超过阈值时，标记为静态
            # static_ratio = static_total / occupied_total if occupied_total > 0 else 0
            # if occupied_total > 0 and static_ratio > 0.4:  # 40%静态占比阈值
            #     refined_dynamic_mask[i] = False

            for dx in [-2, -1, 0, 1, 2]:
                for dy in [-2, -1, 0, 1, 2]:
                    for dz in [-2, -1, 0, 1, 2]:  # 三维邻域
                        ...
                        if 0 <= nx < self.nx and 0 <= ny < self.ny and 0 <= nz < self.nz:
                            ...
                            if is_occupied:
                                ...
                                if not dynamic_grid[nx, ny, nz]:
                                    static_total += 1
            static_ratio = static_total / occupied_total if occupied_total > 0 else 0
            if occupied_total > 0 and static_ratio > 0.4: 
                refined_dynamic_mask[i] = False
...
def visualize_multi_target_tracking(points, velocities, window_name="Multi-Target Tracking"):
    if len(points) == 0:
        return False
...
if __name__ == "__main__":
    # 假设已有加载函数 load_point_cloud_sequence
    # 这里演示在线处理流程
    ...
    filter = OnlineDynamicFilter(
        voxel_size=1.0,
        x_range=(0, 500),      # 根据雷达实际范围调整qqqq
        y_range=(-150, 150),
        z_range=(-5, 50),
        prob_threshold=0.1,     # 占据概率低于30%判为动态
    ...
    for i in range(len(seq['timestamps'])):
        ...
        if len(dyn_pts) > 0:
            ...
            visualize_multi_target_tracking(dyn_pts, dyn_vel, window_name=f"Frame Dynamic Points")